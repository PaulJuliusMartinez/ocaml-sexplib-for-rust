...
// Someday: This should go in src/sexp.rs.
pub struct SexpSerializer;
pub struct CompoundSexpSerializer(Vec<Sexp<'static>>);

pub fn to_sexp<T>(value: &T) -> Result<Sexp<'static>>
where
    T: Serialize,
{
    value.serialize(SexpSerializer)
}

impl ser::Serializer for SexpSerializer {
    type Ok = Sexp<'static>;
    type Error = Error;
    type SerializeSeq = CompoundSexpSerializer;
    type SerializeTuple = CompoundSexpSerializer;
    type SerializeTupleStruct = CompoundSexpSerializer;
    type SerializeTupleVariant = CompoundSexpSerializer;
    type SerializeMap = CompoundSexpSerializer;
    type SerializeStruct = CompoundSexpSerializer;
    type SerializeStructVariant = CompoundSexpSerializer;
        ...
        T: ?Sized + Serialize,
    {
        ...
        let value = value.serialize(SexpSerializer)?;
...
macro_rules! impl_list_sexp_serializer {
    ($serializer:ident, $serialize_elem:ident) => {
        impl ser::$serializer for CompoundSexpSerializer {
            type Ok = Sexp<'static>;
            type Error = Error;
            ...
            where
                T: ?Sized + Serialize,
            {
                let elem = value.serialize(SexpSerializer)?;
                self.push(elem)
            }
...
impl_list_sexp_serializer!(SerializeSeq, serialize_element);
impl_list_sexp_serializer!(SerializeTuple, serialize_element);
impl_list_sexp_serializer!(SerializeTupleStruct, serialize_field);
impl_list_sexp_serializer!(SerializeTupleVariant, serialize_field);

macro_rules! impl_key_value_pair_sexp_serializer {
    ($serializer:ident) => {
        impl ser::$serializer for CompoundSexpSerializer {
            type Ok = Sexp<'static>;
            type Error = Error;
...
impl_key_value_pair_sexp_serializer!(SerializeStruct);
impl_key_value_pair_sexp_serializer!(SerializeStructVariant);

impl ser::SerializeMap for CompoundSexpSerializer {
    type Ok = Sexp<'static>;
    type Error = Error;
        ...
        T: ?Sized + Serialize,
    {
        ...
        let key = key.serialize(SexpSerializer)?;
        self.push(key)
    }
